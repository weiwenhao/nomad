...
Nomad supports interpreting two classes of variables: [node
attributes](#interpreted_node_vars) and [runtime environment
variables](#interpreted_env_vars). Node attributes are interpretable in
[constraints](https://www.nomadproject.io/docs/job-specification/constraint),
[task environment variables](https://www.nomadproject.io/docs/job-specification/env), and certain task driver fields—for example the [`labels`](https://www.nomadproject.io/docs/drivers/docker#labels) attribute of the Docker [`config`](https://www.nomadproject.io/docs/drivers/docker). Runtime
environment variables are not interpretable in constraints because they are only
defined once the scheduler has placed them on a particular node.

## Syntax

The syntax for interpreting variables in the Nomad job specification is
`${variable_name}`. The [`template` stanza](/docs/job-specification/template)
...
interpolation or accessed using the `env` function inside of the template stanza—`{{env "..."}}`

### Dots in Variables ((#dots_in_vars))

Nomad interprets dots in names as object notation. So names that have multiple
consecutive dots are invalid. For example, an environment variable named
`invalid...name` cannot be interpolated using the standard `"${invalid...name}"`
syntax.

Nomad provides a variable—`env`—that can be used to access any environment
variable regardless of name using index syntax. For example: 
